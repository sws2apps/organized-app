...
export const dbLastBRead = async (stuID) => {
    ...
    const lastBRead = appData.filter(data => data.assignmentID === 0 & data.studentID === stuID);
...
export const dbLastIniCall = async (stuID) => {
    stuID = parseInt(stuID, 10);
    const appData = await dbHistoryAssignment();
    const lastIniCall = appData.filter(data => data.assignmentID === 1 & data.studentID === stuID);
...
export const dbLastRV = async (stuID) => {
    stuID = parseInt(stuID, 10);
    const appData = await dbHistoryAssignment();
    const lastRV = appData.filter(data => data.assignmentID === 2 & data.studentID === stuID);
...
export const dbLastBibleStudy = async (stuID) => {
    stuID = parseInt(stuID, 10);
    const appData = await dbHistoryAssignment();
    const lastBibleStudy = appData.filter(data => data.assignmentID === 3 & data.studentID === stuID);
...
export const dbLastTalk = async (stuID) => {
    stuID = parseInt(stuID, 10);
    const appData = await dbHistoryAssignment();
    const lastTalk = appData.filter(data => data.assignmentID === 4 & data.studentID === stuID);
...
export const dbLastAssistant = async (stuID) => {
    stuID = parseInt(stuID, 10);
    const appData = await dbHistoryAssignment();
    const lastAssistant = appData.filter(data => data.assignmentID === 8 & data.studentID === stuID);
    var dLast;
            ...
            for(let i=1; i <= 4; i++) {
                ...
                if (scheduleData[fldStuA] !== undefined || scheduleData[fldStuB] !== undefined) {
                    if (scheduleData[fldStuA] !== "" || scheduleData[fldStuB] !== "") {
                        assType = {};
                        assType.label = await dbGetAssType(sourceData[fldAss]);
        ...
        if (assType === 1) {
            ...
            if (assName === "ass1") {
                if (ass1Type === ass2Type || ass1Type === ass3Type || ass1Type === ass4Type) {
                    s89Data.initialCallSpec = "Anjara 1 (" + assTime + " min.)";
                }
            } else if (assName === "ass2") {
                if (ass2Type === ass1Type || ass2Type === ass3Type || ass2Type === ass4Type) {
                    s89Data.initialCallSpec = "Anjara 2 (" + assTime + " min.)";
                }
            } else if (assName === "ass3") {
                if (ass3Type === ass1Type || ass3Type === ass2Type || ass3Type === ass4Type) {
                    s89Data.initialCallSpec = "Anjara 3 (" + assTime + " min.)";
                }
            } else if (assName === "ass4") {
            ...
            if (assName === "ass1") {
                if (ass1Type === ass2Type || ass1Type === ass3Type || ass1Type === ass4Type) {
                    s89Data.returnVisitSpec = "Anjara 1 (" + assTime + " min.)";
                }
            } else if (assName === "ass2") {
                if (ass2Type === ass1Type || ass2Type === ass3Type || ass2Type === ass4Type) {
                    s89Data.returnVisitSpec = "Anjara 2 (" + assTime + " min.)";
                }
            } else if (assName === "ass3") {
                if (ass3Type === ass1Type || ass3Type === ass2Type || ass3Type === ass4Type) {
                    s89Data.returnVisitSpec = "Anjara 3 (" + assTime + " min.)";
                }
            } else if (assName === "ass4") {
                if (ass4Type === ass1Type || ass4Type === ass2Type || ass4Type === ass3Type) {
                    s89Data.returnVisitSpec = "Anjara 3 (" + assTime + " min.)";
