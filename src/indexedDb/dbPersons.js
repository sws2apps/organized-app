...
export const dbGetStudents = async () => {
    ...
    appData = await appDb.table("persons").reverse().reverse().sortBy("person_name");
    ...
    for(let i=0; i < appData.length; i++) {
        ...
        if (typeof appData[i].viewStudent_Part === "undefined" || appData[i].viewStudent_Part === "" ) {
...
export const dbGetStudentDetails = async (id) => {
    ...
    if (typeof appData.viewStudent_Part === "undefined" || appData.viewStudent_Part === "" ) {
        person.viewStudent_Part = [];
    } else {
...
export const dbGetPersonsByAssType = async (assType) => {
    ...
    if (isLiveClass === true) {
        if (assType === "isAssistant") {
            dbPersons = appData.filter(person => (person.isInitialCall === true || person.isReturnVisit === true || person.isBibleStudy === true) & person.isUnavailable === false & person.forLivePart === true);
        } else {
            dbPersons = appData.filter(person => person[assType] === true & person.isUnavailable === false & person.forLivePart === true);
        }
    } else {
        if (assType === "isAssistant") {
            dbPersons = appData.filter(person => (person.isInitialCall === true || person.isReturnVisit === true || person.isBibleStudy === true) & person.isUnavailable === false);
        } else {
            dbPersons = appData.filter(person => person[assType] === true & person.isUnavailable === false);
        }
    }
    var persons = [];
    ...
    persons.sort((a, b) => {
        ...
        var dateA = a.lastAssignment.split("/")[2] + "/" + a.lastAssignment.split("/")[0] + "/" + a.lastAssignment.split("/")[1];
        var dateB = b.lastAssignment.split("/")[2] + "/" + b.lastAssignment.split("/")[0] + "/" + b.lastAssignment.split("/")[1];
