...
import { getOldestWeekDate, getWeekDate } from '../utils/sourceMaterial';
...
SourcesClass.prototype.removeOutdatedRecords = async function () {
  ...
  const oldSchedules = Schedules.list.filter((schedule) => getWeekDate(schedule.weekOf) < oldestWeekDate);
  for await (const schedule of oldSchedules) {
    await Schedules.delete(schedule.weekOf);
  }

  const oldSources = this.list.filter((source) => getWeekDate(source.weekOf) < oldestWeekDate);
