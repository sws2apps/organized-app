        ...
        >
          <Box sx={{ display: 'flex', flexDirection: 'column' }}>
            <Typography
              ...
              sx={{ minWidth: mobile400Down ? 'unset' : '215px', width: mobile400Down ? 'unset' : '215px' }}
            >
              {buildPersonFullname(
            ...
            {speaker.speaker_data.person_notes.value.length > 0 && (
              <Typography className="label-small-regular">{speaker.speaker_data.person_notes.value}</Typography>
            )}
          </Box>
          ...
          <>
            {!tabletDown && (
              <Button
                ...
                startIcon={<IconPrepareReport width={20} height={20} color="var(--accent-main)" />}
              >
                {t('tr_details')}
              </Button>
            )}
            {tabletDown && (
              <IconButton sx={{ padding: 0 }} onClick={handleOpenSpeakerDetails}>
                <IconPrepareReport width={20} height={20} color="var(--accent-main)" />
              </IconButton>
            )}
          </>
        )}
