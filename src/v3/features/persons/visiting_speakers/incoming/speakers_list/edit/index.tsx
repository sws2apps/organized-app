...
const IncomingSpeakerEdit = ({ speaker }: IncomingSpeakerEditType) => {
  ...
  return (
    <Box sx={{ display: 'flex', gap: '16px', flexDirection: 'column' }}>
      ...
      >
        <TextField label={t('tr_email')} value={email} onChange={(e) => handleEmailChange(e.target.value)} />
        <TextField label={t('tr_phoneNumber')} value={phone} onChange={(e) => handlePhoneChange(e.target.value)} />
      </Box>

      <TextField label={t('tr_shortNote')} value={note} onChange={(e) => handleNoteChange(e.target.value)} />
