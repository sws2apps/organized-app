...
const CongregationOnlineAdd = ({ onCongregationChange }: CongregationOnlineAddType) => {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      <Typography color="var(--grey-400)">{t('tr_addOrganizedCongregationDesc')}</Typography>

      <AutoComplete
        isOptionEqualToValue={(option, value) => option.cong_number === value.cong_number}
        getOptionLabel={(option: IncomingCongregationResponseType) => `${option.cong_name}, ${option.cong_number}`}
        filterOptions={(x) => x}
        autoComplete={true}
        includeInputInList={true}
        options={options}
        renderOption={(props, option) => (
          <Box component="li" {...props} sx={{ margin: 0, padding: 0 }} key={option.cong_number}>
            <Typography>
