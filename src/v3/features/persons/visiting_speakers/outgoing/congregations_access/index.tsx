...
const OutgoingSpeakersAccess = ({ open, onClose }: CongregationAccessType) => {
  ...
  const { congregations, isPending, congregationRevoke, handleSetDelete, handleRevokeAccess } =
    ...
    <Dialog onClose={onClose} open={open} sx={{ padding: '24px' }}>
      {!congregationRevoke && (
        <>
          ...
          {!isPending && congregations.length > 0 && (
            <OutgoingSpeakersListActive congregations={congregations} onDelete={handleSetDelete} />
          )}
      ...
      {congregationRevoke && (
        <Box sx={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <Typography className="h2">{t('tr_congregationAccessRevoke')}</Typography>

          <TextMarkup
            content={t('tr_congregationAccessRevokeDesc', { congregationName: congregationRevoke.cong_name })}
