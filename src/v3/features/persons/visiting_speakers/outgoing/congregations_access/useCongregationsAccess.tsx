...
const useCongregationsAccess = (closeDialog: VoidFunction) => {
  ...
  const [congregationRevoke, setCongregationRevoke] = useState<CongregationRequestType | null>(null);
  ...
  const handleRevokeAccess = async () => {
    try {
      if (!isRevoking) {
        ...
        const { status } = await apiRejectRequestCongregationSpeakers(request_id);
  ...
  return { congregations, isPending, congregationRevoke, handleSetDelete, handleRevokeAccess };
};
