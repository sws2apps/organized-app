...
const ContactItem = ({
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      >
        <TextField label={t('tr_name')} value={name} onChange={(e) => onNameChange(id, e.target.value)} />
        <TextField label={t('tr_contactInfo')} value={contact} onChange={(e) => onContactChange(id, e.target.value)} />
      </Box>
