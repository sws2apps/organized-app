 ...
 */
export const convertDurationInSecondsToHoursAndMinutes = (seconds: number): { hours: number; minutes: number } => {
  const hours = Math.floor(seconds / 3600);
  const minutes = Math.floor((seconds % 3600) / 60);
 ...
 */
export const convertHoursAndMinutesToDurationInSeconds = (time: { hours: number; minutes: number }): number => {
