...
const NotificationItem = ({ notification }: { notification: NotificationRecordType }) => {
    ...
    <Box>
      <Stack mb={2.3} spacing={1}>
        ...
        {notification.type === 'speakers-request' &&
          ...
          notification.options.map((request) => <SpeakerAccessRequest key={request.request_id} request={request} />)}

        <Stack
          direction="row"
          justifyContent={notification.enableRead && !notification.read ? 'space-between' : 'flex-end'}
          alignItems={'center'}
        >
