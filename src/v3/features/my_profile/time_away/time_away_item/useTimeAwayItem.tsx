...
const useTimeAwayItem = (timeAway: TimeAwayType) => {
  ...
  const handleUpdateStartDate = async (value) => {
    ...
    obj.start_date = { value: new Date(value).toISOString(), updatedAt: new Date().toISOString() };
  ...
  const handleUpdateEndDate = async (value) => {
    ...
    const obj = structuredClone(timeAway);
    obj.end_date = { value: new Date(value).toISOString(), updatedAt: new Date().toISOString() };
  ...
  return { start_date, end_date, comments, handleUpdateStartDate, handleUpdateEndDate, handleUpdateComments };
};
