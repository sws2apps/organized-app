...
const AssignmentsDelete = ({ open, onClose, meeting }: AssignmentsDeleteType) => {
  const { t } = useAppTranslation();

  const { handleSetEndWeek, handleSetStartWeek, isProcessing, handleClearAssignments } = useAssignmentsDelete(
    meeting,
    onClose
  );
    ...
    <Dialog onClose={onClose} open={open} sx={{ padding: '24px' }}>
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        <Typography className="h2">
          {meeting === 'midweek' ? t('tr_clearMultipleMM') : t('tr_clearMultipleWM')}
        </Typography>
        <Typography color="var(--grey-400)">{t('tr_clearMultipleDesc')}</Typography>
      </Box>
