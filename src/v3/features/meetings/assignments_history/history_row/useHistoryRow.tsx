...
const useHistoryRow = ({ assignment, isDialog }: HistoryRowType) => {
  ...
  const student = persons.find((record) => record.person_uid === assignment.history_misc.ayf?.student);
  const assistant = persons.find((record) => record.person_uid === assignment.history_misc.ayf?.assistant);

  const history: FormattedHistoryType = {
    history_assignment: assignment.history_assignment,
    history_date: formatDate(new Date(assignment.history_date), t('tr_shortDateFormat')),
    history_hall:
      assignment.history_hall === '1' ? t('tr_hallA') : assignment.history_hall === '2' ? t('tr_hallB') : '',
    history_misc: {
  ...
  const textClassname: CustomClassName = isDialog ? 'body-regular' : 'body-small-semibold';
  const textClassnameAlt: CustomClassName = isDialog ? 'body-regular' : 'body-small-regular';
