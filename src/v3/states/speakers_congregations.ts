...
export const incomingCongSpeakersState = selector({
  ...
  get: ({ get }) => {
    ...
    return incomingCongregations.sort((a, b) => a.cong_data.cong_name.value.localeCompare(b.cong_data.cong_name.value));
  },
});
...
export const congregationsPendingState = selector({
  ...
  get: ({ get }) => {
    ...
    return congregations.filter((record) => record.cong_data.request_status === 'pending');
  },
});
