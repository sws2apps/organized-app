...
export const sourcesFormattedState = selector({
  ...
  get: ({ get }) => {
    ...
    newSources.sort((a, b) => new Date(a.weekOf).getTime() - new Date(b.weekOf).getTime());
    ...
    const groupedData = newSources.reduce<SourcesFormattedType[]>((acc, curr) => {
      ...
      const findMonth = yearRecord.months.find((record) => record.value === month);
      if (!findMonth) {
        yearRecord.months.push({ value: month, weeks: [] });
      }

      // Add current week to the appropriate month array
      const monthRecord = yearRecord.months.find((record) => record.value === month);
