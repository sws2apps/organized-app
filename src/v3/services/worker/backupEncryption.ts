...
import { PersonBackupType, SpeakersCongregationBackupType, VisitingSpeakerBackupType } from './backupType';
...
export const encryptPersonsData = (persons: PersonType[], accessCode, masterKey) => {
...
export const decryptPersonsData = (persons: PersonBackupType[], accessCode, masterKey) => {
  const result = persons.map((person) => {
...
export const encryptSpeakersCongregations = (speakers_congregations: SpeakersCongregationsType[], masterKey) => {
  const result = speakers_congregations.map((congregation) => {
    ...
    obj._deleted = encryptData(JSON.stringify(congregation._deleted), masterKey);
...
export const decryptSpeakersCongregations = (speakers_congregations: SpeakersCongregationBackupType[], masterKey) => {
  const result = speakers_congregations.map((congregation) => {
...
export const encryptVisitingSpeakers = (visiting_speakers: VisitingSpeakerType[], masterKey) => {
