...
const syncFromRemote = <T extends object>(local: T, remote: T): T => {
  const objectKeys = Object.keys(remote).filter((key) => remote[key] !== null && typeof remote[key] === 'object');
  ...
  const primitiveKeys = Object.keys(remote).filter((key) => typeof remote[key] !== 'object');
...
const dbGetTableData = async () => {
  ...
  return { settings, persons, outgoing_speakers, speakers_congregations, visiting_speakers };
};

const dbRestoreFromBackup = async (backupData: BackupDataType, accessCode: string, masterKey?: string) => {
  if (backupData.cong_settings) {
    const settings = await appDb.app_settings.get(1);

    if (backupData.cong_settings.cong_discoverable) {
      if (backupData.cong_settings.cong_discoverable.updatedAt > settings.cong_settings.cong_discoverable.updatedAt) {
        await appDb.app_settings.update(1, {
          'cong_settings.cong_discoverable': backupData.cong_settings.cong_discoverable,
        });
      }
    }
  }
  ...
  if (backupData.cong_persons) {
    ...
    const remotePersons = decryptPersonsData(backupData.cong_persons, accessCode, masterKey);

    for await (const remotePerson of remotePersons) {
      const localPerson = persons.find((record) => record.person_uid === remotePerson.person_uid);
      if (!localPerson) {
  ...
  if (backupData.speakers_congregations) {
    ...
    const remoteCongregations = decryptSpeakersCongregations(backupData.speakers_congregations, masterKey);

    for await (const remoteCongregation of remoteCongregations) {
      const localCongregation = congregations.find((record) => record.id === remoteCongregation.id);
  ...
  if (backupData.visiting_speakers) {
    ...
    const remoteSpeakers = decryptVisitingSpeakers(backupData.visiting_speakers, masterKey);

    for await (const remoteSpeaker of remoteSpeakers) {
      const localSpeaker = speakers.find((record) => record.person_uid === remoteSpeaker.person_uid);
      if (!localSpeaker) {
...
export const dbExportDataBackup = async (userRole: string[], backupData: BackupDataType) => {
  ...
  if (publicTalkEditor) {
    obj.speakers_congregations = encryptSpeakersCongregations(speakers_congregations, masterKey);
    obj.visiting_speakers = encryptVisitingSpeakers(visiting_speakers, masterKey);

    const speakersKey =
      backupData.speakers_key === '' ? generateKey() : decryptData(backupData.speakers_key, masterKey);

    obj.outgoing_speakers = encryptVisitingSpeakers(outgoing_speakers, speakersKey);
