...
import { CustomDivider, DarkOverlay, PageTitle, UserAccountItem } from '@components/index';
    ...
    <>
      <Box sx={{ display: 'flex', gap: '16px', flexDirection: 'column' }}>
        ...
        <Box sx={{ display: 'flex', gap: '16px', flexWrap: desktopUp ? 'nowrap' : 'wrap' }}>
            ...
            >
              ...
              >
                {congregationPersonsList.map((value) => {
                  ...
                  return <UserAccountItem variant="user" userName={value.username} key={randomKey} />;
                })}
              </Box>
            </UsersContainer>
          </Box>
          ...
          <Box sx={{ display: 'flex', gap: '16px', flexDirection: 'column', width: '100%', flexGrow: 1 }}>
            <UsersContainer title={t('tr_baptizedAndAppointed')} description={t('tr_baptizedAndAppointedDesc')}>
              <CustomTypography className="body-small-semibold" color={'var(--black)'}>
              ...
              >
                {baptizedBrothersList.map((value) => {
                  const randomKey = crypto.randomUUID();
                  return <UserAccountItem variant="baptized" userName={value.username} key={randomKey} />;
                })}
              </Box>
            </UsersContainer>
          </Box>
