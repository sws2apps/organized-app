  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <CustomTypography className="h3" color={`var(--group-${props.groupNumber})`}>
          ...
          >
            ...
            <CustomTypography className="body-small-semibold" color={'var(--always-white)'}>
              {props.visitorsCount.toString()}
            </CustomTypography>
          </Box>
        </Box>
        {props.groupName && (
          <CustomTypography className="body-regular" color={`var(--group-${props.groupNumber})`}>
            {props.groupName}
          </CustomTypography>
        )}
      </Box>
      ...
      >
        {props.isMyGroup && <IconMyGroup color={`var(--group-${props.groupNumber})`} />}
...
export const GroupCardContentItem = (props: GroupCardContentItemProps) => {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (displayMenu && groupCardMenuReference.current && !groupCardMenuReference.current.contains(event.target)) {
      ...
      >
        ...
        >
          ...
          {props.descriptionText && (
            <CustomTypography className="label-small-medium" color={'var(--grey-400)'}>
              {props.descriptionText}
            </CustomTypography>
          )}
          {props.thirdText && (
...
export const GroupCardMenu = (props: GroupCardMenuProps) => {
  ...
  return (
    <Popper open={props.open} anchorEl={props.anchorElement} ref={props.reference}>
      <Box
        sx={{
          width: '256px',
...
export const GroupCardDivider = (props: GroupCardDividerProps) => {
  return <CustomDivider color={`rgb(from var(--group-${props.groupNumber}) r g b / 16%)`} />;
};
