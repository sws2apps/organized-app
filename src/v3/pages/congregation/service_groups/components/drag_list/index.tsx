...
import { CustomDragListItemProps, CustomDragListProps } from './drag_list.types';
...
const CustomDragList = (props: CustomDragListProps) => {
  ...
  const CustomDragListItem = (itemProps: CustomDragListItemProps) => {
    ...
    const [{ isDragging }, drag] = useDrag({
      ...
      item: { type: ItemTypes.LIST_ITEM, id: itemProps.value, index: itemProps.index },
      collect: (monitor) => ({
      ...
      >
        ...
        >
          ...
          {variant == 'reorder' ? (
            ...
            <CustomTypography className="body-regular">{itemProps.value}</CustomTypography>
          )}
        </Box>
        {variant == 'publishers' ? (
        ...
        {valuesTexts.map((text, index) => (
          <React.Fragment key={text}>
            <CustomDragListItem
              value={text}
              index={index}
              onDeleteButtonClick={() => setValuesTexts(valuesTexts.filter((_, i) => i !== index))}
            />
