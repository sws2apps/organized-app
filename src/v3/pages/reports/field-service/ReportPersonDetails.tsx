  ...
  return (
    <StyledCardBox sx={{ padding: '24px', borderRadius: 'var(--radius-l)', color: 'var(--black)' }}>
      <HorizontalFlex sx={{ justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Checkbox label={t('tr_lateReport')} checked={isLate} onChange={(e) => setIsLate(e.target.checked)} />
      </HorizontalFlex>
      ...
      {isAPublisher ? (
        ...
        <HorizontalFlex sx={{ justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <TimeField value={totalHours} onChange={(value) => setTotalHours(value)} />
        </HorizontalFlex>
      )}
      <CustomDivider color="var(--accent-200)" />
      <HorizontalFlex sx={{ justifyContent: 'space-between', alignItems: 'center' }}>
        <span className="body-regular">{t('tr_bibleStudies')}</span>
        <CustomNumberField value={nbBibleStudies} onChange={(value) => setNbBibleStudies(value)} />
      </HorizontalFlex>
      <CustomDivider color="var(--accent-200)" />
...
const CustomNumberField = ({ value, onChange }: { value: number; onChange: (value: number) => void }) => {
  const increment = () => {
    onChange(value + 1);
  };
