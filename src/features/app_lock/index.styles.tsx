...
type LockSurfaceProps = BoxProps & { exiting?: boolean };

const withoutExiting = (prop: PropertyKey) => prop !== 'exiting';
...
export const AppLockPage = styled('div', {
  shouldForwardProp: withoutExiting,
})<{
  exiting?: boolean;
}>(({ exiting }) => ({
  position: 'fixed',
  ...
  right: 0,
  bottom: 0,
  // above dialogs (1300), below snackbars (1400)
  zIndex: 1350,
  display: 'flex',
...
})) as unknown as FC<LockSurfaceProps>;

export const AppLockCard = styled('div', {
  shouldForwardProp: withoutExiting,
})<{
  exiting?: boolean;
}>(({ exiting }) => ({
