...
const useSubmitReport = ({ onClose }: SubmitReportProps) => {
  ...
  const handleNextDateUpdate = async (date: string) => {
    ...
    let hours = dailyReport.report_data.hours.split(':').at(0) || '0';
    let minutes = dailyReport.report_data.hours.split(':').at(1) || '0';
    ...
    dailyReport.report_data.hours = `${hours}:${minutes}`;
  ...
  const handleSubmit = async () => {
    await apiUserFieldServiceReportPost({
      ...
      comments: comments,
      hours,
      hours_credits: hours_credits_total,
      approved_assignments,
